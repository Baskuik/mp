    ...
    <div class="min-h-screen grid grid-cols-1 lg:grid-cols-2">
        ...
        <div class="hidden lg:flex flex-col justify-between bg-[#2D6A4F] p-12 relative overflow-hidden">
            ...
            <div class="relative z-10">
                <h1 class="font-display text-5xl text-white leading-tight mb-4">
                    {{ __('messages.register_step3_hero_title') }}<br>
                    <em class="text-[#F4A261] not-italic font-display">{{ __('messages.register_step3_hero_subtitle') }}</em>
                </h1>
                <p class="text-white/60 text-base leading-relaxed max-w-sm">
                    {{ __('messages.register_step3_hero_desc') }}
                </p>
            </div>
                ...
                @foreach ([__('messages.register_step1_label'), __('messages.register_step2_label'), __('messages.register_step3_label')] as $index => $title)
                    <div class="flex flex-col items-center gap-2">
                        <div
                            ...
                            @else bg-white/40 text-white @endif
        ...
        <div class="flex items-center justify-center px-6 py-12 lg:px-16">
            <div class="w-full max-w-md">
                ...
                {{-- Header --}}
                <div class="mb-8">
                    <h2 class="font-display text-3xl text-gray-900 mb-2">{{ __('messages.register_step3_heading') }}</h2>
                    <p class="text-gray-500 text-sm">{{ __('messages.register_step3_subheading') }}</p>
                </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-xl px-6 py-6 mb-8">
                    <div class="flex gap-4">
                        ...
                        <div>
                            <h3 class="text-sm font-medium text-blue-900">{{ __('messages.register_step3_check_email') }}</h3>
                            <p class="text-sm text-blue-700 mt-2">
                                {{ __('messages.register_step3_sent_to') }} <strong>{{ $email }}</strong> {{ __('messages.register_step3_code_info') }}
                            </p>
                            <p class="text-sm text-blue-600 mt-3">
                                {{ __('messages.register_step3_code_validity') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" action="/verify-email-code" class="space-y-5">
                    ...
                    @if ($errors->has('code'))
                        <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                            <p class="text-sm text-red-700">{{ $errors->first('code') }}</p>
                        </div>
                    @endif
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                            {{ __('messages.register_step3_code_label') }}
                        </label>
                        <input type="text" name="code" maxlength="6"
                            placeholder="{{ __('messages.register_step3_code_placeholder') }}"
                            ...
                            value="{{ old('code') }}">
                        <p class="text-xs text-gray-500 mt-2">{{ __('messages.register_step3_code_helper') }}</p>
                    </div>
                    ...
                    <div class="flex gap-3 pt-4">
                        ...
                        <button type="submit"
                            ...
                            {{ __('messages.register_step3_button') }}
                        </button>
                    </div>
                </form>
                ...
                <div class="text-center mt-6">
                    <p class="text-sm text-gray-600">{{ __('messages.register_step3_resend') }}</p>
                    <form method="POST" action="{{ route('resend-verification-email') }}" class="inline">
                        ...
                        <button type="submit" class="text-[#2D6A4F] font-medium hover:underline text-sm">
                            {{ __('messages.register_step3_resend_button') }}
                        </button>
                    </form>
                </div>
            </div>
